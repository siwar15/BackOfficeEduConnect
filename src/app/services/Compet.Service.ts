...
export class CompetService {
  ...
  addUserToCompetition(competitionId: number, userId: number): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/competition/${competitionId}/user/${userId}`, {});
  }

  isUserAlreadyRegistered(competitionId: number, userId: number): Observable<boolean> {
 ...
 /* sendNotificationByEmail(email: string): Observable<string> {
  ...
  sendNotificationByEmail(email: string, userId: number, friendId: number): Observable<string> {
  ...
  getUserFriendIdByEmail(email: string): Observable<number> {
